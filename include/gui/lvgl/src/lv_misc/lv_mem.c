 ...
 **********************/
#if LV_MEM_CUSTOM == 0
    static uint8_t * work_mem __attribute__((section(".data")));
#endif

static uint32_t zero_mem __attribute__((section(".data"))); /*Give the address of this variable if 0 byte should be allocated*/

#if LV_MEM_CUSTOM == 0
    static uint32_t mem_max_size __attribute__((section(".data"))); /*Tracks the maximum total size of memory ever used from the internal heap*/
#endif

static uint8_t mem_buf1_32[MEM_BUF_SMALL_SIZE] __attribute__((section(".data")));
static uint8_t mem_buf2_32[MEM_BUF_SMALL_SIZE] __attribute__((section(".data")));

static lv_mem_buf_t mem_buf_small[] __attribute__((section(".data"))) = {{.p = mem_buf1_32, .size = MEM_BUF_SMALL_SIZE, .used = 0},
...
void _lv_mem_init(void)
{
    ...
    for(int i = 0; i < LV_MEM_SIZE / sizeof(MEM_UNIT); i++) {
        work_mem_int[i] = 0;
    }
