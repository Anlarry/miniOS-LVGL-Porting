 ...
 *  STATIC VARIABLES
 **********************/
static bool lv_initialized __attribute__((section(".data"))) = false;
...
static int _memcmp(const void* _p1,const void* _p2, int size) {
    const uint8_t* p1 = _p1;
    const uint8_t* p2 = _p2;
    if(p1 == NULL || p2 == NULL) 
        return -1;
                                       ...
                                       lv_state_t new_state)
{
    ...
    if((prop & 0xF) < LV_STYLE_ID_COLOR) { /*Int*/
        ...
        if(_memcmp(&p1, &p2, sizeof(const void *)) == 0)  return NULL;
        obj->state = prev_state;
...
static style_snapshot_res_t style_snapshot_compare(style_snapshot_t * shot1, style_snapshot_t * shot2)
{
    if(_memcmp(shot1, shot2, sizeof(style_snapshot_t)) == 0) return STYLE_COMPARE_SAME;
